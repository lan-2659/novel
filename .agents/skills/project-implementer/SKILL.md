...
项目根目录必须存在 `README.md`。它是反映项目当前真实样貌的快照，必须让一个完全不熟悉项目的开发者仅凭 README 就能理解项目结构、模块职责和运行逻辑。

### 12.1 README 必须包含的章节

1. **项目整体描述**  
   用 2～3 句话说明项目是做什么的、解决什么问题、核心流程是什么。不得出现模糊或夸张描述。

2. **技术栈**  
   以表格或列表形式列出前端、后端、存储、LLM、流式等使用的具体技术。

3. **目录结构（逐文件说明）**  
   必须列出项目根目录下所有主要文件和文件夹（可省略虚拟环境、`__pycache__`、`.git` 等无关内容），并对**每一个文件或子目录**给出**一句话作用说明**。  
   格式示例：
   ```text
   project/
   ├── backend/
   │   ├── app/
   │   │   ├── main.py          # FastAPI 入口：创建应用、挂载静态文件、注册路由
   │   │   ├── config.py        # 读取环境变量
   │   │   └── ...
   ...
   └── README.md
   ```
   必须根据实际目录结构生成，不得虚构或遗漏关键文件。

4. **快速开始**  
   提供从环境配置到启动项目的最小步骤，包括安装依赖、配置环境变量、启动命令、访问地址。若前端无需构建，需明确说明。

5. **分模块解释**  
   对项目中的**主要模块**（如 API 层、Service 层、LLM 客户端、状态机、上下文构建器、数据模型、Repository 层等）逐一解释。每个模块必须包含：
   - **模块名称**
   - **作用**：一句话说明该模块的职责。
   - **实现原理**：用自然语言 + 箭头描述这个模块接收什么数据 → 对这些数据做什么处理 → 最终输出什么内容，让人能快速理解模块的业务逻辑，而不需要深入代码细节。
   示例：
     ```text
     接收用户的创意和当前阶段 → 从数据库中读取已有的设定、大纲、章节规划 → 将这几部分拼接成完整的提示词 → 输出给大模型生成新的内容
     ```
   - **对应 API**：列出该模块直接处理或间接支持的 HTTP 接口（如果有）。若模块不暴露 HTTP 接口（如 Repository），说明它被哪些 Service 或 API 调用。

### 12.2 生成 README 的信息来源

AI 必须在生成 README 前：
- 扫描项目根目录及子目录，获取真实文件结构。
- 若项目已有 README，应更新而非完全重写，保留正确内容，修正过时或错误信息。

### 12.3 真实性原则

- 只能描述已经被代码和测试证明存在的功能。
- 禁止把“计划中的功能”写成“已实现”。未完成的必须标记“未实现”或“部分完成”。
- 目录结构和文件作用必须与代码事实完全一致。
- 模块的实现原理必须与真实调用链一致，不得凭空编造。
- 每次修改代码后必须同步更新 README，文档永远不能落后于代码。
